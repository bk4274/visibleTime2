...
//        selectedArrayList = new ArrayList<>();
        ...
        Log.d(tag,"CreateRoutineActivity - onCreate() | controllerPlanDataArrayList.size(): "+controllerPlanDataArrayList.size());
...
//                arrayListParentName.add(catagoryData.getParentName());
        ...
        builder.setAdapter(arrayAdapter, new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int position) {
                if(position == arrayListName.size()-1){
                    Toast.makeText(CreateRoutineActivity.this, "추가", Toast.LENGTH_SHORT).show();
                    Intent intent = new Intent(CreateRoutineActivity.this, CategoryActivity.class);
                    ...
                    for(int i=0; i<arrayListIndexOfParent.size(); i++){
                        if(position == arrayListIndexOfParent.get(i)){
                            ...
                            Toast.makeText(CreateRoutineActivity.this, "상위 활동은 선택하 실 수 없습니다.", Toast.LENGTH_SHORT).show();
                            isParentType = true;
                            break;
                        } else {
                            // 하위 활동인 경우
    ...
    public void enterBtnClicked(){
        if(routineNameTextView.getText().toString().length() == 0){
            Toast.makeText(CreateRoutineActivity.this, "'활동 목표'의 이름을 정해주세요.", Toast.LENGTH_SHORT).show();
        } else {
            ...
            if(isOverlap == true){
                Toast.makeText(CreateRoutineActivity.this, "현재 활동목표 이름은 기존에 존재하는 값이므로 사용하실 수 없습니다.", Toast.LENGTH_SHORT).show();
            } else{
                // 컨트롤 데이터 저장
                controllerPlanDataArrayList.add(new ControllerPlanData(routineNameTextView.getText().toString(),false));
                // 데이터 이름 저장.
...
//    }
    ...
    /**getCurrentDate()
     * 주석 처리한 이유, 해당 Activity에서는 Plan Data 틀을 만드는 곳이고, 실제로 사용되는 Plan Data는 Routine Activity에서 실행된다.*/
...
//        mAdapter.notifyDataSetChanged();
    ...
    public void controllRecyclerViewItem(){
        ...
        mAdapter.setOnItemClickListener(new PlanAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View v, int position) {
                ...
                AlertDialog.Builder mBuilder = new AlertDialog.Builder(CreateRoutineActivity.this);
                ...
                mBuilder.setPositiveButton("네", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialogInterface, int i) {
                        ...
                        if(useableTime>=targetTime || time > targetTime || useableTime+time >=targetTime){
                            ...
                            Toast.makeText(CreateRoutineActivity.this, "사용 가능한 시간은 "+useabletimeDuration+"입니다.", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
    ...
    private void savePlanFrameDataList(){
        Log.d(tag,"CreateRoutineActivity - savePlanFrameDataList() | ");
        Log.d(tag,"CreateRoutineActivity - savePlanFrameDataList() | planDataArrayList.size: "+planDataArrayList.size());
        SharedPreferences sharedPresesntID = getSharedPreferences("presentID", MODE_PRIVATE);
        String presentID = sharedPresesntID.getString("presentID",null);
        ...
        SharedPreferences sharedPreferences = getSharedPreferences(presentID, MODE_PRIVATE);
        SharedPreferences.Editor editorPlanFrame = sharedPreferences.edit();
...
//        editorCategory.commit();
        ...
        if(planDataArrayList.size() != 0){
            for(int i=0; i<planDataArrayList.size(); i++){
                ...
                if (plan == null) {
                    // 첫 카테고리 정보를 저장하는 경우
                    editorPlanFrame.putString("planFrame", strDataOfJSON);
                    editorPlanFrame.commit();
                } else {
                    // 카테고리 정보가 처음이 아닌 경우
                    editorPlanFrame.putString("planFrame", sharedPreferences.getString("planFrame", "") + "," + strDataOfJSON);
                    editorPlanFrame.commit();
                }
            }
        }
    ...
    private void saveControllerPlanData(){
        Log.d(tag,"CreateRoutineActivity - saveControllerPlanData() |  ");
